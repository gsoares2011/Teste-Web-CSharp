...
namespace Teste_Web_CSharp1.Facade
{
    public class PontoTuristicoFacade
    {
        ...
        {
            if (PontoTuristicoBuilder.NovoPontoTuristico().comNome(nome).comDescricao(descricao).comEndereco(localizacao).comCidade(cidade).comEstado(estado).Construir().GravarPontoTuristico())
            {
